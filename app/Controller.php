...
class Controller{
    private $access_token  = "eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjo4OTIzNSwidGltZXN0YW1wIjoiMjAxOS0xMS0yMSAxNDoyMjo0MiArMDcwMCJ9.Sd8DOaJE6Bp5hHeItyPstBGXppQFWjPCYT1m3aT9oKs"; //akses token dapat diambil dari sini https://qisme.qiscus.com/app/kiwari-prod
    ...
    private function replyCommandTextLastBuild($display_name,$message_type,$room_id){
            // persiapkan curl
    $ch = curl_init(); 
    curl_setopt($ch, CURLOPT_URL, "https://script.google.com/macros/s/AKfycbwkYERUze6UTW4OrBrVqqHIY4OBI-VWrPOLHDTTlcsSRVghYAI/exec");
...
CATERING : IKA CATERING";
    ...
    //$json = json_decode($output, true);
        ...
        // $output = file_get_contents(__DIR__ . '/../app/test.json');
        // $json = json_decode($output, true);
    
    // if($json == null){
    //     echo json_last_error() . "<br>";
    //     echo "null"; // good to check what the source was, to see where it went wrong
    // }else{
    //     echo $json['Version'];
    // }
        ...
        if($data->getMessage() != null && $find_slash !== false){
            ...
            if(isset($command[1])){
                switch($command[1]){
                    ...
                    //     break;  
                    case 'lastbuildandroid':
                        $this->replyCommandTextLastBuild($data->getSender(),$data->getMessageType(),$data->getRoomId());
                        break;
                    default:
